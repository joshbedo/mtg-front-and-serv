...
export const gameEmitters = {
  ...
  // Card actions
  playCard: (card: Card, position: { x: number; y: number }, tapped: boolean, playerName: string) => {
    const roomId = gameStorage.getRoomId();
    if (!roomId) return;
  ...
  toggleCard: (cardId: string, tapped: boolean, playerName: string, card: Card) => {
    const roomId = gameStorage.getRoomId();
    if (!roomId) return;

    socket.emit('game:cardTapped', { roomId, cardId, tapped, playerName, card });
  },
  ...
  // Life total actions
  updateLife: (playerId: string, life: number, delta: number, playerName: string) => {
    const roomId = gameStorage.getRoomId();
    if (!roomId) return;
