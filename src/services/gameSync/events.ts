...
export const initializeGameStateEvents = () => {
  ...
  socket.on('game:phaseChanged', ({ phase, playerName }) => {
    ...
    useChatStore.getState().addMessage(`${playerName} moved to ${phase} phase`, undefined, undefined, 'phase');
  });

  socket.on('game:turnChanged', ({ playerId, playerName }) => {
    useGameStore.getState().setCurrentPlayer(playerId);
    useChatStore.getState().addMessage(`${playerName}'s turn`, undefined, undefined, 'system');
  });
};
...
export const initializeBattlefieldEvents = () => {
  socket.on('game:cardPlayed', ({ card, position, tapped, cardId, playerName }: {
    ...
    useChatStore.getState().addMessage(`${playerName} played ${card.name}`, card);
  });
  ...
  socket.on('game:cardTapped', ({ cardId, tapped, playerName, card }: {
    cardId: string;
    tapped: boolean;
    playerName: string;
    card: Card;
...
export const initializeHandEvents = () => {
  ...
  socket.on('game:cardDrawn', ({ playerName, count }: {
    playerName: string;
    count: number;
  }) => {
    useChatStore.getState().addMessage(
...
export const initializeLifeEvents = () => {
  socket.on('game:lifeChanged', ({ playerId, life, delta, playerName }: {
    ...
    useChatStore.getState().addMessage(
      `${playerName} ${action} ${Math.abs(delta)} life (${prevLife} → ${life})`,
      undefined,
      undefined,
      'life'
    );
...
export const initializePileEvents = () => {
  socket.on('game:cardToGraveyard', ({ card, pileId, playerName }: {
    card: Card;
    pileId: string;
    playerName: string;
  }) => {
    ...
    useChatStore.getState().addMessage(
      `${playerName} moved ${card.name} to graveyard`,
      card
    );
  });

  socket.on('game:cardExiled', ({ card, pileId, playerName }: {
    card: Card;
    pileId: string;
    playerName: string;
  }) => {
