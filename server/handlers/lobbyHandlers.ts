...
export const handleLobbyActions = (io: Server, socket: Socket) => {
  ...
  socket.on('lobby:create', ({ name, player }: { name: string; player: Player }) => {
  ...
  socket.on('lobby:join', ({ lobbyId, player }: { lobbyId: string; player: Player }) => {
    const playerWithSocket = { ...player, socketId: socket.id };
  ...
  socket.on('lobby:leave', ({ lobbyId, playerId }: { lobbyId: string; playerId: string }) => {
    ...
    if (lobby) {
      io.to(lobbyId).emit('lobby:update', lobby);
      io.to(lobbyId).emit('chat:system', `${socket.data?.playerName} left the game`);
    }
